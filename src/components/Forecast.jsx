...
export function Forecast({ store }) {
  const { invitees, cancelRate, setCancelRate, contingency, setContingency, attendees, grandTotal, perAttendee, fixedTotal, varTotal, varCosts } = store;
  const { isMobile } = useResponsive();

  return (
    <div>
      <div style={{ fontFamily: "'Playfair Display', serif", fontSize: isMobile ? 22 : 26, fontWeight: 600, color: C.ink, marginBottom: 28 }}>Pronostico</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 1, marginBottom: 1 }}>
        <div style={{ ...glassCard, padding: isMobile ? 20 : 28 }}>
          ...
          <SliderInput label="Cancelaciones" value={cancelRate} min={0} max={40} onChange={setCancelRate} format={(v) => `${v}%`} accent={C.green} />
          ...
          <div style={{ borderTop: `1px solid ${C.stone}`, paddingTop: 16, marginTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: isMobile ? 28 : 34, fontWeight: 600, color: C.ink }}>{attendees}</span>
          </div>
        </div>
