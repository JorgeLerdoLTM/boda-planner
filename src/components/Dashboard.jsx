...
export function Dashboard({ store, cx }) {
  const {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "1fr 1fr 1fr 1fr", gap: 1, marginBottom: isMobile ? 20 : 32 }}>
        <KpiCard label="Total Estimado" value={f(grandTotal)} sub={`incl. ${contingency}% cont.`} highlight />
        <KpiCard label="Pagado" value={f(paidTotal)} sub={`${Math.round((paidTotal / grandTotal) * 100)}%`} />
        <KpiCard label="Balance" value={f(balance)} sub="pendiente" />
        <KpiCard label="Por Asistente" value={f(perAttendee)} sub={`${attendees} esp.`} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 1, marginBottom: 1 }}>
        ...
        <div style={{ ...glassCard, padding: isMobile ? 20 : 28 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            <Pill label={`Fijo ${f(fixedTotal)}`} bg={C.blueLt} color={C.blue} />
            <Pill label={`Variable ${f(varTotal)}`} bg={C.yellowLt} color="#8B6914" />
          </div>
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
              ...
              <Tooltip formatter={(v) => f(v)} contentStyle={{ border: `1px solid ${C.stone}`, borderRadius: 0, fontSize: 12, fontFamily: "'Inter', sans-serif" }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ ...glassCard, padding: isMobile ? 20 : 28, marginBottom: 1 }}>
        ...
        <ResponsiveContainer width="100%" height={Math.max(200, catBreakdown.length * (isMobile ? 32 : 28))}>
          <BarChart data={catBreakdown} layout="vertical" margin={{ left: 10, right: isMobile ? 20 : 40 }}>
            ...
            <XAxis type="number" tickFormatter={(v) => fs(v)} style={{ fontSize: 10 }} />
            <YAxis type="category" dataKey="name" width={isMobile ? 80 : 120} style={{ fontSize: isMobile ? 10 : 11 }} />
            <Tooltip formatter={(v) => f(v)} contentStyle={{ border: `1px solid ${C.stone}`, borderRadius: 0, fontSize: 12 }} />
        ...
        <div style={{ ...glassCard, padding: isMobile ? 20 : 28 }}>
          ...
          {upcoming.map((c, i) => (
            <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 0", borderBottom: i < upcoming.length - 1 ? `1px solid ${C.stone}` : "none" }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: C.ink, fontFamily: "'Playfair Display', serif" }}>{f(Number(c.amount) - Number(c.paid || 0))}</div>
            </div>
          ))}
        </div>
      </div>
